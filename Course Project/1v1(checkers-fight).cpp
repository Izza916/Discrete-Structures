...
// Player class for data collection
class Player
{
public:
    string name;
    ...
    Player(const string &playerName, char piece) : name(playerName), piece(piece), upperPiece(toupper(piece)) {}
};

list<Player> players;

// Using Stack for Collecting winners name
stack<string> winnersStack;
...
{
    cout << "Winners:\n";
    while (!winnersStack.empty())
    {
        cout << winnersStack.top() << " ";
        winnersStack.pop();
    }
    cout << endl;
}
...
play_again:
    ...
    players.push_back(Player(p1, p1[0]));
    ...
    players.push_back(Player(p2, p2[0]));
...
bool check_move()
{
    //  a non-king piece is moving backwards
    ...
    // the location have space or not
    ...
    // location entered by the user contains a piece to be moved
    ...
    // the piece isn't moving diagonally
    ...
    //  the piece is moving by more than 1 column and only 1 row
    ...
    if (row2 > row1 + 1 || row2 < row1 - 1)
    {
        ...
        //  the piece isn't moving by exactly 2 columns
        ...
        // the piece is leaping over another piece from the other player
        char otherPlayer;
...
// [e.g izza wins] Determine if the game is over
void game_over()
{
    int counter = 0;
